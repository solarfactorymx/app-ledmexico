...
def main(page: ft.Page):
    ...
    # ⚠️ REEMPLAZA CON TU ENLACE DE RENDER
    URL_SERVIDOR = "https://motor-led-mexico.onrender.com"

    # ⚠️ REEMPLAZA CON EL NOMBRE DE TU ARCHIVO JSON DE GOOGLE
                ...
                if db_usuario == usuario_login:
                    if db_password == password_login:
                        ...
                        try:
                            ...
                            return False, f"❌ Error de fecha en Drive (Usa AAAA-MM-DD)."
    ...
    # ==========================================
    # 1. MOTOR OCR — FILEPICKER + CÁMARA IA
    # ==========================================
    txt_estado_ocr = ft.Text("Listo para escanear recibo...", color="grey", italic=True)
    prg_ocr = ft.ProgressBar(width=300, visible=False)

    def _aplicar_datos_ocr(datos):
        """Rellena la interfaz con los datos que regresó cualquier endpoint OCR."""
        if datos.get("tarifa_detectada"):
            actualizar_interfaz(datos["tarifa_detectada"])
        ...
        if consumos_detectados:
            for campo in entradas_historial:
                campo.value = "0"
                campo.color = "white"
    ...
    # --- FilePicker (PDF / imagen desde galería) ---
        ...
        try:
            ...
            with open(ruta_archivo, 'rb') as f:
                ...
                try:
                    ...
                    datos = {"error": f"El servidor respondió con código {respuesta.status_code}."}

            if datos.get("exito"):
                txt_estado_ocr.value = "✅ ¡Recibo procesado con éxito!"
                txt_estado_ocr.color = "green"
                _aplicar_datos_ocr(datos)
            else:
    ...
    def procesar_archivo_seleccionado(e: ft.FilePickerResultEvent):
        if getattr(e, "files", None) is None or not e.files:
            return
        archivo = e.files[0]
        txt_estado_ocr.value = "⏳ Enviando a la nube Render..."
    ...
    btn_ocr = ft.ElevatedButton(
        "📂 SUBIR PDF / IMAGEN",
        bgcolor="#145A32", color="white", height=50,
    ...
    # --- Botón Cámara (IA Vision) ---
    async def abrir_camara(e):
        txt_estado_ocr.value = "📷 Abriendo cámara..."
        txt_estado_ocr.color = "orange"
        prg_ocr.visible = True
        btn_camara.disabled = True
        page.update()

        try:
            # Abre la cámara trasera del teléfono, sube la foto y espera el JSON
            resultado_raw = await page.run_javascript(f"""
                new Promise((resolve, reject) => {{
                    ...
                    input.onchange = async (event) => {{
                        ...
                        try {{
                            const response = await fetch('{URL_SERVIDOR}/api/ocr-camara', {{
                                method: 'POST',
                                body: formData
                            }});
            ...
            if not resultado_raw:
                txt_estado_ocr.value = "⚠️ Se canceló la foto."
                txt_estado_ocr.color = "grey"
                prg_ocr.visible = False
            ...
            if datos.get("exito"):
                txt_estado_ocr.value = "✅ ¡Foto procesada con IA Vision!"
                txt_estado_ocr.color = "green"
                _aplicar_datos_ocr(datos)
            else:
                txt_estado_ocr.value = f"❌ Error IA: {datos.get('error', 'desconocido')}"
                txt_estado_ocr.color = "red"

        except Exception as ex:
            txt_estado_ocr.value = f"❌ Error cámara: {str(ex)}"
            txt_estado_ocr.color = "red"
    ...
    contenedor_ocr = ft.Container(
        content=ft.Column([
            ft.Text("Escanear Recibo CFE", weight="bold", color="#2ECC71"),
            ft.Row(
                [btn_ocr, btn_camara],
    ...
    contenedor_historial = ft.Container(
        content=ft.Column([
            ...
            ft.Column(entradas_historial, spacing=5, scroll="auto", height=380)
        ]),
    ...
    campos_costos = {
        ...
        "GDMTO": [crear_c("Suministro", "80"), crear_c("Distribución", "0"), crear_c("Capacidad", "0"),
                  crear_c("Transmisión", "0"), crear_c("CENACE", "0"), crear_c("SCnMEM", "0"), crear_c("Energía Neta", "0")],
    ...
    def actualizar_interfaz(t):
        ...
        for k in campos_costos:
            if isinstance(campos_costos[k], list):
                for c in campos_costos[k]:
                    c.color = "white"
            else:
                campos_costos[k].color = "white"

        if t in ["01", "1A", "1B", "1C", "1D", "1E", "1F"]:
            columna_costos.controls.extend(campos_costos["01"])
            switch_mensual.value = False
        elif t == "DAC":
            columna_costos.controls.extend(campos_costos["DAC"])
            switch_mensual.value = False
        elif t == "PDBT":
            columna_costos.controls.extend(campos_costos["PDBT"])
            switch_mensual.value = False
        elif t == "GDMTO":
            columna_costos.controls.extend(campos_costos["GDMTO"])
            switch_mensual.value = True
        elif t == "GDMTH":
            columna_costos.controls.extend(campos_costos["GDMTH"])
            switch_mensual.value = True
        ...
        img_g1.visible = False
        img_g2.visible = False
        page.update()

    def btn_t(txt):
        return ft.ElevatedButton(txt, on_click=lambda _: actualizar_interfaz(txt), bgcolor="#2874A6", color="white")

    grid_tarifas = ft.Column([
        ft.Row([btn_t("01"), btn_t("1A"), btn_t("1B"), btn_t("1C"), btn_t("1D")], scroll="auto"),
        ft.Row([btn_t("1E"), btn_t("1F"), btn_t("DAC"), btn_t("PDBT"), btn_t("GDMTO"), btn_t("GDMTH")], scroll="auto")
    ])
    ...
    def calcular_propuesta(e=None):
        ...
        try:
            ...
            if not consumos:
                res_final.content.value = "⚠️ Error: Ingresa consumos."
            ...
            paquete = {
                "tarifa": tarifa_activa.value, "potencia_panel_w": w_panel,
                "cantidad_paneles": cant_final, "hsp": hsp, "eficiencia": eficiencia,
                "es_mensual": switch_mensual.value, "consumos": consumos, "costos": dict_costos
            }
            ...
            if d.get("exito"):
                ...
                datos_pdf_global.update({
                    "tarifa": tarifa_activa.value, "paneles": cant_final, "watts": w_panel,
                    "promedio": round(promedio_kwh, 2), "ahorro_anual": ahorro_anual,
                    "pago_promedio": sum(p_list) / len(p_list), "gen_kwh": gen_kwh,
                    "es_mensual": switch_mensual.value, "potencia_inst": potencia_inst,
                    "inversor_sug": inversor_sug, "gen_diaria": gen_diaria_total,
                    "gen_anual": gen_anual, "co2": co2, "hsp": hsp,
                    "c_list": c_list, "p_list": p_list, "a_list": a_list,
                    "np_list": np_list, "b_list": b_list
                })
                ...
                try:
                    ...
                    ax1.set_xticks(x)
                    ax1.set_xticklabels(etiquetas, color='white')
                    ...
                    fig1.savefig(ruta_g1, format='png')
                    plt.close(fig1)
                    ...
                    ax2.set_xticks(x)
                    ax2.set_xticklabels(etiquetas, color='white')
                    ...
                    fig2.savefig(ruta_g2, format='png')
                    plt.close(fig2)
                    ...
                    print("Advertencia gráficas:", ex_graf)

                res_final.content.value = (
                    f"✅ CALCULO COMPLETADO:\n"
                    f"Paneles: {cant_final} | Potencia: {round(potencia_inst, 2)}kWp | "
                    f"Ahorro Anual: ${round(ahorro_anual, 2):,}"
                )
                res_final.bgcolor = "#145A32"
                btn_pdf.visible = True
            else:
                res_final.content.value = f"Error del Servidor: {d.get('error')}"
                res_final.bgcolor = "#7B241C"
        except Exception as ex:
            res_final.content.value = f"Error de cálculo: {ex}"
            res_final.bgcolor = "#7B241C"
        page.update()
    ...
    def generar_y_compartir_pdf(e):
        ...
        res_final.content.value = "⏳ Construyendo documento PDF..."
        page.update()
        ...
        try:
            ...
            for i in range(7):
                pdf.cell(anchos[i], 7, cabeceras[i], border=1, align="C", fill=True)
            pdf.ln()
            ...
            pdf.set_font("Arial", '', 10)
            texto_p = (
                f"Considerando su consumo y para cubrir los gastos reflejados en el recibo de CFE, "
                f"se determina la necesidad de instalar {datos_pdf_global.get('paneles', '')} paneles solares "
                f"de {datos_pdf_global.get('watts', '')} Watts de potencia. Este sistema solar fotovoltaico "
                f"tendra la capacidad de generar aproximadamente {datos_pdf_global.get('gen_kwh', 0):,.0f} kWh "
                f"por periodo. En consecuencia, se propone la instalacion de un inversor con una capacidad de "
                f"al menos {datos_pdf_global.get('inversor_sug', 0):.2f} kW para gestionar eficientemente la "
            ...
            try:
                if os.path.exists(ruta_g1):
                    pdf.image(ruta_g1, x=25, y=None, w=160, type='PNG')
            except Exception:
                pass
            pdf.ln(10)
            try:
                if os.path.exists(ruta_g2):
                    pdf.image(ruta_g2, x=25, y=None, w=160, type='PNG')
            except Exception:
                pass
    ...
    pantalla_principal = ft.Column([
        ...
        fila_graficas_web,
        ft.Column(
            [btn_calcular, btn_pdf, btn_abrir_pdf],
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
            alignment=ft.MainAxisAlignment.CENTER
        ),
        res_final
